# How data migration works — in plain language

This explains the machinery without jargon. The reference tables (which thing
becomes which OptiERP thing) live in [DATA_MIGRATION.md](DATA_MIGRATION.md);
...
Your old system knows things by **name** ("ABC Traders"). We know things by **ID**
(a long unique code). So importing is really one question, asked a few thousand
times: *which of our records does this name mean?* — plus the bookkeeping to make
sure a wrong answer is cheap to fix.

A spreadsheet adds one question in front of it: *which sheet is the invoice
list, and which column is the amount?* That one you answer once, and we remember
it.
...
| Warehouse move | Data migration |
|---|---|
| The truck | Your export file — Tally XML, or an Excel workbook from anywhere |
| Reading the manifest on the truck | Working out which sheet is what (spreadsheets only) |
| The loading bay | Staging rows (`migration_staging_records`) |
| Labelling each box | Mapping each name in the file to one of our records |
...
## "Why isn't this sheet imported?"

Open the import, go to **Sheets & columns**, and expand any sheet marked *not
imported*. It tells you which of three things is true:

- **We have no module for it.** Projects, timesheets, payroll. Named as such.
- **We have no concept for it.** A second classification axis, a "what-if" set of
  books. The value is kept on the staging row so nothing is lost.
- **Your file doesn't carry enough.** A Zoho backup's order sheets have totals but
  no line items; the orders themselves import perfectly when the lines are there.

There is a fourth case that reads like a gap and is not: things we refuse on
purpose. Your company setup, your user list and your old audit trail are all
deliberately left alone, and the sheet says why. An imported audit trail would
claim that people who have never used this system made changes at times nothing
here can corroborate.

Anything you disagree with, you can override: pick a shape for the sheet in the
same panel and it imports.
