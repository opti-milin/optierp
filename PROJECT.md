...
- **Latest migration head:** `0102_data_migration_rename` (after `0101_meeting_serial_per_type`, `0100_secretarial_capital`, `0098`–`0099` tally, `0094`–`0097` secretarial).
...
- **Module 12 is now source-agnostic Data Migration** (`0102`). What was "Tally Import" imports **any application's `.xlsx`** as well as Tally XML/CSV: a workbook adapter detects the shape (Tally full dump, Tally flat sheet, Zoho Books backup, the OptiERP template), a mapping wizard covers everything else, and a saved company profile makes an unrecognised export a one-time cost. Everything below the parser is unchanged — one intermediate representation, so a Zoho invoice gets the same GL posting, dedupe and rollback as a Tally voucher. Tables/routes/services renamed `tally_*` → `migration_*`; UI at `/data-migration/*` with `/tally/*` redirecting. Coverage then widened to every target that already existed: party addresses and contact people (from an inline Tally ledger or their own joined sheets), payment terms templates, bank accounts and statement lines, tax templates, currencies and rates, price lists, budgets, fixed assets, quotations, and the Customer/Supplier fields no profile could previously name (payment terms, tax category, group, territory, credit limit and days, registered name, notes, disabled). Every sheet still not imported now states which of four things is true — no module, no concept, the file does not carry it, or deliberately refused — and the wizard shows that sentence instead of a bare "not imported". Sample exports in [docs/migration/](docs/migration/). See [docs/DATA_MIGRATION.md](docs/DATA_MIGRATION.md) and [docs/plans/data_migration_spreadsheet.plan.md](docs/plans/data_migration_spreadsheet.plan.md).
