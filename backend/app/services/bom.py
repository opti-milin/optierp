...
async def _build_item_rows(
    ...
    *,
    ...
    # A row that IS the finished good is the clearest case to name, so check it before
    # the multi-level detector — that walks the component BOMs and would report the
    # generic "cycle detected" for what is really a one-line mistake.
    for idx, r in enumerate(rows, start=1):
        if r.item_id == production_item_id:
            raise ValidationError(
                f"Component row {idx}: '{items[r.item_id].item_code}' is the BOM's own "
                "finished good — a BOM cannot contain itself",
                field="items",
            )
