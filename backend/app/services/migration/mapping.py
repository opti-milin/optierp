...
That answer lives in ``MigrationMapping`` rows, and this module is what fills them:
...
from app.models.migration import MAPPING_TARGETS, MigrationMapping
from app.services.migration.catalogue import ENTITY_BY_KEY, normalise_unit
...
def normalise_name(name: str | None) -> str:
    ...
    if not name:
        return ""
    # "&" and "and" are the same word to a human, and the two spellings collide
    # constantly: Tally ships "Reserves & Surplus" while our Indian Chart of
    # Accounts calls it "Reserves and Surplus". Stripping "&" as punctuation left
    # "reserves surplus" against "reserves and surplus" — no match, and a
    # near-duplicate account created beside the real one.
    text = _AMPERSAND.sub(" and ", name.casefold())
...
def best_match(
    source_name: str, candidates: Sequence[Candidate]
...
async def list_mappings(
    ...
    *,
    ...
    stmt = select(MigrationMapping).where(MigrationMapping.company_id == company_id)
    if entity_key:
        stmt = stmt.where(MigrationMapping.entity_key == entity_key)
    if unresolved_only:
        stmt = stmt.where(MigrationMapping.target_id.is_(None))
    if search:
        stmt = stmt.where(MigrationMapping.source_name.ilike(f"%{search}%"))
    stmt = stmt.order_by(MigrationMapping.entity_key, MigrationMapping.confidence, MigrationMapping.source_name)
    return list((await db.execute(stmt)).scalars().all())


async def get_mapping(
    db: AsyncSession, company_id: uuid.UUID, entity_key: str, source_name: str
) -> MigrationMapping | None:
    stmt = select(MigrationMapping).where(
        MigrationMapping.company_id == company_id,
        MigrationMapping.entity_key == entity_key,
        MigrationMapping.source_name == source_name,
    )
...
async def upsert_mapping(
    ...
    *,
    ...
    existing = await get_mapping(db, company_id, entity_key, source_name)
    if existing is not None:
        ...
        existing.source_guid = source_guid or existing.source_guid
        existing.source_parent = source_parent or existing.source_parent
        if attributes:
...
async def set_mapping(
    ...
    *,
    ...
    mapping = await db.get(MigrationMapping, mapping_id)
...
async def auto_map_entity(
    ...
    ``names`` yields ``(source_name, source_guid, source_parent)``. Existing locked
    ...
    seen: set[str] = set()
    for source_name, source_guid, source_parent in names:
        ...
        # renamed, GUIDs don't.
        prior = by_guid.get(source_guid) if source_guid else None
        if prior is not None and prior.target_id is not None:
            await upsert_mapping(
                db, company_id, user,
                entity_key=entity_key, source_name=cleaned, source_guid=source_guid,
                source_parent=source_parent, target_doctype=prior.target_doctype,
        ...
        await upsert_mapping(
            db, company_id, user,
            entity_key=entity_key, source_name=cleaned, source_guid=source_guid,
            source_parent=source_parent, target_doctype=target,
...
class MappingRef:
    ...
    #: primary key of the migration_mappings row, when one is persisted
...
class MappingBook:
    ...
    def _put(self, ref: MappingRef) -> None:
        ...
        bucket[ref.source_name.strip().casefold()] = ref
    ...
    async def bind(
        ...
        if ref.row_id is not None:
            await db.execute(
                sa_update(MigrationMapping)
                .where(MigrationMapping.id == ref.row_id)
                .values(
                    target_id=target_id,
