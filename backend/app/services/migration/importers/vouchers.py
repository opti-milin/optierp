...
from app.schemas.selling import QuotationCreate, SalesOrderCreate
...
from app.services import (
    ...
    quotation as quotation_service,
...
from app.services.migration.catalogue import DUTY_HEADS, VOUCHER_TYPES, normalise_unit
        ...
        bills.append(
            {
                ...
                # "30 Days" — Tally's credit term for this bill. Without it an
                # imported invoice has no due date, so it never ages and never
                # appears as overdue.
                "credit_period": text_of(bill, "BILLCREDITPERIOD"),
            }
        )
    return bills


#: "30 Days", "45 days", "1 Month" — Tally writes the credit term as free text.
_CREDIT_PERIOD = re.compile(r"^\s*(\d+)\s*(day|week|month)s?\s*$", re.I)
_PERIOD_DAYS = {"day": 1, "week": 7, "month": 30}


def _due_date(posting_date: date, rows: list[LedgerRow]) -> date | None:
    """Due date from the party row's bill-wise credit period, if Tally gave one."""
    for row in rows:
        if not row.is_party:
            continue
        for bill in row.bills:
            match = _CREDIT_PERIOD.match(str(bill.get("credit_period") or ""))
            if match:
                return posting_date + timedelta(
                    days=int(match.group(1)) * _PERIOD_DAYS[match.group(2).lower()]
                )
    return None
...
def _posting_date(context: ImportContext, record: MigrationStagingRecord) -> date:
    if record.posting_date:
        return record.posting_date
    raise ValidationError("The voucher has no date")


def _remarks(record: MigrationStagingRecord) -> str:
    """Narration plus a breadcrumb back to the Tally voucher."""
    narration = (record.raw or {}).get("narration") or ""
    tag = f"[Tally {record.source_voucher_type} {record.voucher_number or ''}]".strip()
...
async def import_journals(context: ImportContext, records: list[MigrationStagingRecord]) -> None:
...
def _invoice_lines(
    context: ImportContext, record: MigrationStagingRecord, rows: list[LedgerRow], inventory: list[InventoryRow]
) -> list[InvoiceItemIn]:
...
def _invoice_taxes(
    ...
    *,
    ...
    Whether a tax row adds to the invoice or comes off it is *not* readable from
    its sign alone. Which side a tax ledger sits on depends on the voucher:

    ==================  ==============================  ============
    Voucher             Tax ledger in Tally             Effect here
    ==================  ==============================  ============
    Sales               Output GST — credit             Add
    Purchase            Input GST — debit (your ITC)    Add
    Credit Note         reverses a sale — debit         Add
    Debit Note          reverses a purchase — credit    Add
    ==================  ==============================  ============

    So the ordinary case is always "Add", on whichever side the voucher puts it,
    and only a row sitting on the *opposite* side to its voucher is a genuine
    deduction — TDS withheld on a purchase bill, say. Reading "debit means
    deduct" instead made every purchase subtract its GST: a 1,00,000 bill with
    18,000 IGST came out at 82,000 rather than 1,18,000, and understated the
    supplier's payable by the same amount.

    Lines are imported at absolute value with the return carried as a flag on
    the document, so a return's tax follows the same rule — hence the flip.
    """
    natural_side_is_debit = (kind == "Purchase") != is_return
    ...
    for row in rows:
        ...
        taxes.append(
            TaxRowIn(
                ...
                add_deduct_tax=(
                    "Add" if (row.debit > ZERO) == natural_side_is_debit else "Deduct"
                ),
            )
        )
    return taxes
...
def _party_ledger(record: MigrationStagingRecord, rows: list[LedgerRow]) -> str | None:
...
async def import_invoices(
    context: ImportContext, records: list[MigrationStagingRecord], *, kind: str
) -> None:
    ...
    for record in records:
        async with context.row(record):
            if _skip_cancelled(context, record):
                continue
            spec = VOUCHER_TYPES.get(record.source_voucher_type or "")
            ...
            taxes = _invoice_taxes(context, rows, kind=kind, is_return=is_return)
            posting_date = _posting_date(context, record)
            ...
            if kind == "Sales":
                ...
                payload = SalesInvoiceCreate(
                    due_date=_due_date(posting_date, rows),
...
def _reference_date(data: dict[str, Any]) -> date | None:
    from app.services.migration.sources.tally_xml import parse_tally_date
...
async def _credit_note_as_journal(
    context: ImportContext, record: MigrationStagingRecord, rows: list[LedgerRow]
) -> None:
...
async def import_payments(context: ImportContext, records: list[MigrationStagingRecord]) -> None:
    for record in records:
        async with context.row(record):
            if _skip_cancelled(context, record):
                continue
...
async def import_orders(
    ...
    """``kind`` is ``"Quotation"``, ``"Sales"`` or ``"Purchase"``.

    All three need item lines and none of them post to the ledger, which is what
    makes them one function. The refusal below is the important line: an order
    or quotation is *what was ordered*, so a source that ships only the header
    totals has not given us the document. Importing it anyway would create
    something nobody can fulfil, match a receipt against, or convert.
    """
    for record in records:
        async with context.row(record):
            if _skip_cancelled(context, record):
            ...
            if not inventory:
                raise ValidationError(
                    f"The {kind.casefold() if kind == 'Quotation' else kind.casefold() + ' order'} "
                    "has no item lines, and its lines are what it is. Map the "
                    "sheet holding them, or leave that sheet unimported."
                )
            party_name = _party_ledger(record, _ledger_rows(data))
            posting_date = _posting_date(context, record)
            ...
            items = [
                OrderItemIn(
                    ...
                    uom=row.uom,
                    warehouse_id=context.book.warehouse(row.godown),
                    delivery_date=due if kind in ("Sales", "Quotation") else None,
            ...
            if kind == "Quotation":
                ...
                if context.option("submit_vouchers", True):
                    quote = await quotation_service.submit_quotation(
                        context.db, quote.id, context.user
                    )
                context.created(record, "Quotation", quote.id, quote.name)
            elif kind == "Sales":
                customer_id = context.book.customer(party_name)
                if customer_id is None:
                    raise ValidationError(f"'{party_name}' is not mapped to a Customer")
...
async def import_stock_journals(context: ImportContext, records: list[MigrationStagingRecord]) -> None:
...
async def import_physical_stock(context: ImportContext, records: list[MigrationStagingRecord]) -> None:
...
async def skip_entity(
    context: ImportContext, records: list[MigrationStagingRecord], reason: str
) -> None:
...
def _skip_cancelled(context: ImportContext, record: MigrationStagingRecord) -> bool:
