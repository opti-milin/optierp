...
from app.services.tally.catalogue import ENTITY_BY_KEY, classify_voucher
...
def parse_tally_date(value: str | None) -> date | None:
    """Tally dates are ``YYYYMMDD``; some exports use ``DD-MMM-YYYY``."""
    if not value:
        return None
    text = value.strip()
...
def parse_amount(value: str | None) -> Decimal:
    """Tally amounts carry sign, currency symbol and thousands separators.
    ...
    if not text:
        return Decimal("0")
    # "₹ -1,234.56" / "(-)1234.56" / "1,234.56 Dr"
    negative = text.startswith("-") or "(-)" in text or text.rstrip().endswith("Dr")
...
def as_list(value: Any) -> list[Any]:
    """Tally emits one child as a dict and many as a list — normalise to a list."""
    if value is None:
        return []
...
def text_of(node: Any, *keys: str, default: str | None = None) -> str | None:
    """Read the first present key from a parsed node, tolerating ``{'#text': ...}``."""
...
class ParsedFile:
    """What one Tally export contained, bucketed by catalogue entity key."""
    ...
    def __init__(self) -> None:
        ...
        #: Entities present in the file that the catalogue does not cover.
    ...
    for message in root.iter("TALLYMESSAGE"):
        for child in message:
            ...
            if tag == "VOUCHER":
                ...
                result.add(_MASTER_TAGS[tag], _master_record(child, _MASTER_TAGS[tag], sequence))
            elif tag == "COMPANY":
                result.company_name = child.get("NAME") or text_of(element_to_dict(child), "NAME")
            else:
...
def _master_record(element: ElementTree.Element, entity_key: str, sequence: int) -> dict[str, Any]:
    ...
    return {
        ...
        "name": name.strip(),
        "parent": (element.get("RESERVEDNAME") or text_of(data, "PARENT") or "").strip() or None,
        "data": data,
    }
...
def _voucher_record(element: ElementTree.Element, sequence: int) -> dict[str, Any] | None:
    ...
    return {
        ...
        "date": parse_tally_date(text_of(data, "DATE")),
...
def parse_csv(text: str, *, entity_key: str = "voucher_journal") -> ParsedFile:
    ...
    for row in rows[header_row + 1 :]:
        if not any((c or "").strip() for c in row):
            continue
        sequence += 1
        posting_date = parse_tally_date(cell(row, "date")) or carried_date
