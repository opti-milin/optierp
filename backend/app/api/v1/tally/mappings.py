...
from app.services.tally import mapping as mapping_service

router = APIRouter(prefix="/tally/mappings", tags=["data migration: tally"])
...
@router.get(
    "",
    response_model=list[TallyMappingResponse],
...
async def list_mappings(
    ...
    return [TallyMappingResponse.model_validate(m) for m in rows[:limit]]
...
@router.post(
    "",
    response_model=TallyMappingResponse,
    status_code=201,
...
async def create_mapping(
    ...
    await db.commit()
    return TallyMappingResponse.model_validate(row)
