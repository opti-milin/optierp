...
import type { MigrationImportEntity, MigrationStagingList, MigrationStagingRecord } from "@/types/migration";

const props = defineProps<{ importId: string; entities: MigrationImportEntity[] }>();

const rows = ref<MigrationStagingRecord[]>([]);
const total = ref(0);
const offset = ref(0);
const limit = 100;
...
async function load(): Promise<void> {
  ...
  try {
    const data = (
      await api.get<MigrationStagingList>(`/migration/imports/${props.importId}/records`, {
        params: {
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody>
          ...
          <template v-for="row in rows" :key="row.id">
            ...
            >
              <td class="px-4 py-2">
                <div class="font-medium text-gray-900">{{ row.source_name || "—" }}</div>
                <div class="text-xs text-gray-400">
                  {{ row.entity_key }}<span v-if="row.source_parent"> · under {{ row.source_parent }}</span>
                </div>
              </td>
