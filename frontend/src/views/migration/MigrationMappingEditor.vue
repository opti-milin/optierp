...
import type { MigrationAutoMapResult, MigrationMapping, MigrationMappingTarget } from "@/types/migration";

const props = defineProps<{ automap: MigrationAutoMapResult | null }>();
const emit = defineEmits<{ changed: [] }>();

const rows = ref<MigrationMapping[]>([]);
...
const editing = ref<MigrationMapping | null>(null);
const targets = ref<MigrationMappingTarget[]>([]);
...
function tone(row: MigrationMapping): string {
...
async function load(): Promise<void> {
  ...
  try {
    rows.value = (
      await api.get<MigrationMapping[]>("/migration/mappings", {
        params: {
...
async function openEditor(row: MigrationMapping): Promise<void> {
  editing.value = row;
...
async function loadTargets(): Promise<void> {
  ...
  targets.value = (
    await api.get<MigrationMappingTarget[]>("/migration/mappings/targets", {
      params: {
...
async function save(): Promise<void> {
  ...
  error.value = null;
  try {
    await api.put(`/migration/mappings/${editing.value.id}`, {
      target_doctype: editing.value.target_doctype,
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody>
          ...
          <tr v-for="row in rows" :key="row.id" class="border-b border-gray-100">
            <td class="px-4 py-2 font-medium text-gray-900">{{ row.source_name }}</td>
            <td class="px-4 py-2 text-gray-500">{{ row.source_parent || "—" }}</td>
    ...
    >
      <div class="w-full max-w-lg rounded-lg bg-white p-5 shadow-lg">
        <h3 class="text-sm font-semibold text-gray-900">
          Map "{{ editing.source_name }}" to a {{ editing.target_doctype }}
        </h3>
