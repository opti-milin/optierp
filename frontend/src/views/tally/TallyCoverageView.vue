...
// The coverage matrix — "what exactly comes across from Tally?".
...
import type { TallyCatalogue } from "@/types/tally";

const catalogue = ref<TallyCatalogue | null>(null);
...
onMounted(async () => {
  loading.value = true;
  try {
    catalogue.value = (await api.get<TallyCatalogue>("/tally/catalogue")).data;
  } catch (e) {
...
<template>
  <div>
    <div class="mb-4">
      <RouterLink to="/tally" class="text-sm text-blue-600 hover:underline">← Imports</RouterLink>
      <h1 class="text-xl font-semibold text-gray-900">What comes across from Tally</h1>
      <p class="max-w-3xl text-sm text-gray-500">
        Every Tally entity the importer understands and the record it becomes here. Anything
        marked <em>Reference only</em> or <em>Not yet</em> is still read and counted, so nothing
        in your Tally file disappears without being reported.
      </p>
    </div>
